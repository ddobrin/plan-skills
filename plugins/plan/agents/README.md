...
This directory (`plugins/plan/agents/` and root `agents/`) packages the 13 standalone subagent definitions (`<agent-name>/agent.md`) for Jetski and Antigravity (`agy`), complementing the skills under [`plugins/plan/skills/`](../skills/). All 13 subagents are strictly governed by the declarative contracts in [`plugins/plan/graph.json`](../graph.json) (`plan-swarm@2.1`) and verified by `python3 plugins/plan/lib/graph/graph.py validate --agents-dir plugins/plan/agents`.
...
[`plugins/plan/graph.json`](../graph.json) is the static schema validated by `python3 plugins/plan/lib/graph/graph.py validate --agents-dir plugins/plan/agents`:
