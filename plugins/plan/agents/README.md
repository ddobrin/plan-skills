...
This directory is an **Antigravity-format (`agy`)** version of the planning swarm that lives alongside the plugin skills at [`plugins/plan/`](../README.md). 
...
├── supervisor/agent.md                 # Orchestrator (paired with starter skill)
...
#### 1. `supervisor` (paired with `starter`) — The Project Manager
...
- **Owns:** Protocol enforcement, artifact management, human gating, and the git protocol (sole committer in the swarm).
...
- **Constraints:** Never fixes code (reports findings, hands fixes back to the engineer); no new capability without tests = automatic FAIL; **never runs `git commit`** (the Supervisor (`supervisor` / `starter`) is the sole committer, and commits only on a **passing audit AND explicit user approval**).
...
- **Machinery:** Convene 3 delegates (intent · codebase · delivery by default), each deep-reading only its territory. Every claim must cite its territory (`file:line`, spec clause, or CI command); sequential verbatim-relayed turns (`send_message` or full-transcript re-invocation), hard cap of 4 rounds.
...
- **System One (TypeSafe Jev):** Provides sub-second classification, discrete alignment, and probability estimation via `plugins/plan/tools/typesafe_validator_engine.py`:
  1. **Pre-flight Sieve (<250ms):** Fast-screens for untestable buzzwords, missing error paths, or unverifiable steps before spawning heavy skeptics.
  2. **Semantic Deduplication & Quorum:** Clusters findings across skeptics using TypeSafe semantic Choice alignment, eliminating fragile kebab-case string collisions.
  3. **1-Vote Tail SDE Cascade:** Automatically evaluates 1-vote tail findings; promotes high-confidence solo catches ($P \ge 0.85$) and filters noise ($P < 0.40$) without requiring expensive deliberator mini-panels.
  ...
  5. **First Domino Ranking:** Ranks plan findings deterministically by evaluating cascading failure impact.
...
- **TypeSafe Acceleration:** Fast preflight sieve flags unquantified buzzwords in advisory mode; findings are semantically clustered into quorum.
...
- **TypeSafe Acceleration:** Evaluates cascading failure risk to pinpoint the definitive **`first_domino`** — the earliest step whose failure topples downstream tasks.
- **Output:** Each finding cites `file:line` evidence and a `fix`; the panel names the **`first_domino`**.
...
Antigravity CLI discovered agents are installed either **as a plugin**, **globally**, or **workspace-locally**. Follow one of the methods below to make the plan swarm available on your system.

### Method 1: Install via `plugins/plan` (Recommended)
Because `plugins/plan` already bundles both `skills/` and `agents/` alongside `plugin.json`, you can install the entire plugin directly from the repository root:
```bash
agy plugin install ./plugins/plan
```

### Method 2: Loose Global Agents
...
| `subagent_type: "general-purpose"` skeptic dispatch | Invokes subagent with `TypeName: research` (or `research-google` / `self`) for read-only skeptics/delegates. |
...
| `SendMessage` across deliberation rounds | Continues the running delegate via `send_message` (`Recipient: "<conversationID>"`), or simulates a persistent channel by re-invoking the delegate each round with the **full verbatim transcript**. |
